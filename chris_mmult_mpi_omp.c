...
int main(int argc, char* argv[])
{
    ...
    if (argc > 2){ // user successfully passed in two file names for two matrices
        if (myid==0){
            ...
            //file 1
            if(fgets(line, sizeof line, a_data)!= NULL)
            {
                token = strtok(line, p1);
                ...
                while(token != NULL)
                {
                    value[i] = malloc(strlen(token) + 1);
                    strcpy(value[i], token);
                    i++;
                    token = strtok(NULL,p1);
                }

                char *rowValue = value[1];
                int rLen = strlen(rowValue);
                rowValue[rLen - 6] = '\0';
                a_nrows = atoi(rowValue);
                printf("int rows: %d\n", a_nrows);

                char *colValue = value[2];
                int cLen = strlen(colValue);
                colValue[cLen - 1] = '\0';
                a_ncols = atoi(colValue);
                printf("int cols: %d\n", a_ncols);
            }

            //populate matrix(aa) 1
            aa = (double*)malloc(sizeof(double) * a_nrows * a_ncols)
            for(i = 0; i < a_nrows; i++)
            {
                for(j = 0; j < a_ncols; j++)
                {
                    int hold = fscanf(a_data, "%1f", &matrix1[i * a_ncols + j]);
                }
            }
            fclose(a_data);

            //file2
            if(fgets(line, sizeof line, b_data)!= NULL)
            {
                token = strtok(line, p1);
                ...
                int rLen = strlen(rowValue);
                rowValue[rLen - 6] = '\0';
                b_nrows = atoi(rowValue);
                printf("int rows: %d\n", b_nrows);

                char *colValue = value[2];
                int cLen = strlen(colValue);
                colValue[cLen - 1] = '\0';
                b_ncols = atoi(colValue);
                printf("int cols: %d\n", b_ncols);
            }

            //populate matrix(bb) 2
            bb = (double*)malloc(sizeof(double) * b_nrows * b_ncols);
            for(i = 0; i < b_nrows; i++)
            {
                for(j = 0; j < b_ncols; j++)
                {
                    int hold = fscanf(fp1, "%1f", &matrix1[i * b_ncols + j]);
                }
            }
